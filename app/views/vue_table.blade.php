...
@section('content')
  ...
  <ul class="pagination">
   <li class="page-item"><a class="page-link" href="{{$sitePath}}main_vue/{{ ($page-1) > 0 ? ($page-1) : 1 }}"><</a></li>
  @foreach($pageCount as $n)
  
    <li class="page-item"><a class="page-link {{ $page == $n ? 'active' : '' }}" href="{{$sitePath}}main_vue/{{$n}}">{{$n}}</a></li>
 
    @endforeach
       <li class="page-item"><a class="page-link" href="{{$sitePath}}main_vue/ {{ ($page+1) < count($pageCount) ? ($page+1) : count($pageCount) }}">></a></li>
  </ul>
</nav> 
...
const App = {
  ...
  methods:{
    sortBy(type){
         ...
         axios.get('{{$sitePath}}main_json/{{$page}}/type/'+this.orderType)
        .then(response => {
    ...
    getData(){
        axios.get('{{$sitePath}}main_json/{{$page}}')
        .then(response => {
        this.data = response.data;
